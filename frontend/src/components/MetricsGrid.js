          ...
          allMetrics.map(async (metric) => {
            try {
              ...
              if (latestValueData && latestGoalData) {
                ...
                statusColor =
                  status === "Good" ? STATUS_COLORS.GOOD : STATUS_COLORS.BAD;
              }
              ...
              console.error(
                `Error fetching data for metric ID ${metric.id}:`,
                innerError
              );
              return {
                ...metric,
              ...
              title={
                <>
                  ...
                  <Typography variant="body2">
                    Latest Value:{" "}
                    {metric.latestValue !== null ? metric.latestValue : "N/A"}
                  </Typography>
                  <Typography variant="body2">
                    Latest Goal:{" "}
                    {metric.latestGoal !== null ? metric.latestGoal : "N/A"}
                  </Typography>
                  <Typography variant="body2">
